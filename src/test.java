...
public class test{
    ...
    public static void main(String[] args) throws Exception {
        ...
        int[] gray = {15, 30, 45, 60, 75, 90, 105, 120, 135, 150, 165, 180, 195, 210, 225, 240};
//
    for(int num = 1; num <= counter; num++) {
        ...
        for(int y = 0; y < raster.getHeight(); y++) { 
            ...
            boolean notGray = true;
            if (ArrayUtils.contains(gray, red)) {
                if (red==green&&red==blue) {
                    notGray = false;
                }
            }

            if (!ArrayUtils.contains(redTrue, red)&&notGray) {
                int cnum = 0;
                int dist = 255;
                for (int i = 0; i < redTrue.length; i++) {
                    if (Math.abs(redTrue[i]-red)<dist) {
                        dist = Math.abs(redTrue[i]-red);
                        cnum = redTrue[i];
                    }
                }
            red = cnum;
            }
            if (!ArrayUtils.contains(greenTrue, green)&&notGray) {
                int cnum = 0;
                int dist = 255;
                for (int i = 0; i < greenTrue.length; i++) {
                    if (Math.abs(greenTrue[i]-green)<dist) {
                        dist = Math.abs(greenTrue[i]-green);
                        cnum = greenTrue[i];
                    }
                }
            green = cnum;
            }
            if (!ArrayUtils.contains(blueTrue, blue)&&notGray) {
                int cnum = 0;
                int dist = 255;
                for (int i = 0; i < blueTrue.length; i++) {
                    if (Math.abs(blueTrue[i]-blue)<dist) {
                        dist = Math.abs(blueTrue[i]-blue);
                        cnum = blueTrue[i];
                    }
                }
            blue = cnum;
        ...
        String background = String.format("%02X",red)+String.format("%02X",green)+String.format("%02X",blue);

                if ((red+green+blue)!=0)Sline += String.format("%s", background + String.format("%02X", x+1) + String.format("%02X", y+1));
            }
        }
